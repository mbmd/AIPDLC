...
> **Scope:** Cross-cutting — spans AI-PILC, AI-ADLC, AI-DWG, AI-GCE, and the AI-DLC runtime
> **Created:** 2026-06-09
> **Author:** Maheri
> **Related:** `FAMILY_STRUCTURE.md`, idea `005-lifecycle-change-propagation` (ownership dimension folded in), siblings `003`/`004`
...
```
PIP (AI-PILC) → AP (AI-ADLC) → DW (AI-DWG) → enforcement layer (AI-GCE) → operated software (AI-DLC sessions)
```
...
| **P4 — Package-dev workspace** | the `ai-packages/*` source itself | persona (via steering fileMatch) | `.kiro/steering/*-rules.md` | — (convention only) | **Weak** |
...
3. **Enforceable package-dev ownership.** A `CODEOWNERS`-style mapping for `ai-packages/*` binding each package to its persona/owner, enforced by a pre-edit governance hook rather than convention alone (closes G2).
