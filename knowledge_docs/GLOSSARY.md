...
## A. Project Management & Initiation (AI-PILC (Project Initiation Life Cycle))

| Term | Definition |
|------|-----------|
| **Project Initiation Package (PIP)** | The complete governance foundation for a project — all initiation artifacts assembled and approved, ready for execution handoff. AI-PILC's final output that feeds the Project layer (→AI-POLC (Product Ownership Life Cycle) → AI-UXD (UX Design) → AI-ADLC (Architecture Design Life Cycle)). |
...
| Term | Definition |
|------|-----------|
| **Architecture Package (AP)** | Complete technical design foundation for a project — all architecture artifacts assembled, validated, and ready for AI-DWG (Workspace Generator) to transform into a development workspace. AI-ADLC's final output. |
...
| Term | Definition |
|------|-----------|
| **Development Workspace (DW)** | Ready-to-code project environment: →Steering Files, folder structure, configuration files, operational documents, planning templates. AI-DWG's output — what AI-DLC (AI-Driven Development Life Cycle — Amazon's open-source build lifecycle) builds within. |
...
| **Downstream Signaling** | Notification from AI-DWG to →AI-GCE (Governance & Compliance Engine) indicating workspace files were generated or updated, triggering compliance re-derivation. Implemented via marker file presence/timestamps. |
...
| **Session Governance** | AI-DLC methodology rules: spec-before-code, never →Vibe Code, one task at a time, correction escalation protocol. Enforced on every prompt submission. |
...
| **Phase Gates (PG-*) — build lifecycle (AI-GCE/AI-DLC)** | Milestone gates with specific pass criteria: Setup→Foundation, Foundation→Construction, Construction→Integration, Integration→Go-Live. Distinct from →Phase Gate (initiation lifecycle — AI-PILC). |
...
| **Governance Agent** | AI-triggered governance checkpoint invoked at process milestones via shortcut command (e.g., `SDC__` (Session Discipline), `SGV__` (Sprint Governance)). Three types: Infrastructure, Process, Audit. |
...
## E. Test Governance (AI-TGE (Test Governance Engine))
...
| **Observation Phase** | AI-TGE's continuous mode: watching AI-DLC build execution, tracking what gets tested, updating register status and coverage reports. Non-blocking — informs, doesn't gate. |
...
| Term | Definition |
|------|-----------|
| **AI-* Family** | The complete suite of injectable workflow packages: AI-ILC (Idea Life Cycle), AI-PILC, AI-PPM (Project Portfolio Management) (Portfolio layer) + AI-ADLC, AI-UXD, AI-POLC, AI-DWG, AI-GCE, AI-TGE (Project layer) + AI-FLO (Flow Orchestrator) (Edge router). |
...
| **Project Layer** | AI-* Family scope covering ONE project end-to-end: AI-POLC (product ownership), AI-UXD (UX), AI-ADLC (design), AI-DWG (workspace), AI-GCE (compliance), AI-TGE (test governance), AI-DLC (build). Runs sequentially: POLC→UXD→ADLC→DWG. |
...
| **Shortcut Trigger** | Pattern `{3-LETTER-UPPERCASE}__` (three capitals + double underscore) to invoke →Governance Agents. Examples: `SDC__`, `SGV__`, `IQA__` (Initiation Quality), `BLH__`. Globally unique per workspace. |
...
| **AI-DLC (AI-Driven Development Life Cycle)** | Amazon's open-source build lifecycle ([awslabs/aidlc-workflows](https://github.com/awslabs/aidlc-workflows)). NOT part of the AI-* Family — but the AI-* chain produces the workspace AI-DLC consumes and builds within. |
