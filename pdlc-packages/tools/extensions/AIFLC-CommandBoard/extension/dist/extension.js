...
function activate(context) {
    const openCmd = vscode.commands.registerCommand('aiflc-commandboard.open', () => {
        ...
        panel = vscode.window.createWebviewPanel('aiflcCommandBoard', 'AIFLC Command Board', vscode.ViewColumn.Beside, {
            enableScripts: true,
...
function buildHTML(context) {
    ...
    if (vscode.workspace.workspaceFolders) {
        for (const folder of vscode.workspace.workspaceFolders) {
            ...
            candidates.forEach(p => { if (!possiblePaths.includes(p))
    ...
    if (!cssContent) {
        cssContent = 'body { font-family: sans-serif; padding: 20px; }';
    }
    if (!jsContent) {
        jsContent = 'document.body.innerHTML="<p>Command board files not found.</p>";';
    }
    return `<!DOCTYPE html>
