...
> **Amendment (2026-07-30 — INT-1 shared integration surface):** adds `integrations/{target}/{in,out}/` at the project root (sibling of `management_framework/`) as a family-wide canonical, **opt-in**, create-if-absent surface for external-tool integration (§4a). **Derived + disposable** — deleting it loses no authoritative data. First consumer: AI-UXD ↔ Figma.
...
└── pdlc-ws/                                    ◄ PDLC FAMILY WORKSPACE (all family output nests here)
    ...
    │   │   ├── integrations/   ◄ shared external-tool surface (opt-in; §4a) · integrations/{target}/{in,out}/
    │   │   │
    │   │   └── {slug}-workspace/               ══ DEV WORKSPACE (generated by AI-DWG)
    │   │       │                                  opened SEPARATELY in its own IDE to build
    │   │       ├──.governance/                ◄ AI-GCE + AI-TGE single home (engine, rules, hooks, agents, test)
    │   │       │   ├── engine/ai-gce/ · ai-tge/   ◄ companion cores (provisioned by DWG Q3) [OI-204]
    │   │       │   ├── workspace-manifest.yaml     ◄ discovery contract
...
### 4a. Shared Integration Surface — `integrations/` (INT-1)

External-tool integration is **not** duplicated inside each package. All packages share ONE standardized surface at the **project root**, a sibling of `management_framework/`:

```
pdlc-ws/projects/PRJ-{ABBREV}-{slug}/
└── integrations/                  ← shared external-tool surface (all targets, all packages)
    ├── README.md                  ← the in/out convention (create-if-absent)
    └── {target}/                  ← e.g. figma/ · storybook/ · penpot/ …
        ├── in/                    ← incoming: tool → package (an INPUT)
        └── out/                   ← outgoing: package → tool (a derived projection)
```

- **Family-wide canonical, opt-in.** The `integrations/{target}/{in,out}/` shape is defined once as a family-wide canonical pattern; **participation is opt-in** — only packages that need it wire it (today: AI-UXD ↔ Figma). AI-ILC (`pdlc-ws/ideas/`) and AI-PPM (`pdlc-ws/portfolio/`) never operate in the per-project tree, so they cannot reach it.
- **Structure family-owned; behavior package-owned.** This contract defines the folder shape, `in`/`out` semantics, and create-if-absent. Each package defines — in its OWN workflow — what it writes to `out/`, reads from `in/`, when, and in what format.
- **`in` / `out` semantics.** `out/` = forward *publish* (package → tool); `in/` = reverse *input* (tool → package, ingested as an input source — not drift).
- **Derived + disposable (invariant).** A package's own folder holds its information in full; the `integrations/` surface is always derived from it. **Deleting `integrations/` loses no authoritative data** — it regenerates on the next run (create-if-absent; never scaffolded empty).
...
| Dev workspace | `pdlc-ws/projects/PRJ-{ABBREV}-{slug}/{slug}-workspace/` | one folder = one project | opened **separately** in its own IDE | `.governance/workspace-manifest.yaml`, `.governance/engine/ai-gce/` |
...
| Integrations | `pdlc-ws/projects/PRJ-{ABBREV}-{slug}/integrations/{target}/{in,out}/` | **multi** (opt-in) | workspace IDE | `integrations/README.md` (derived + disposable) |
...
| Integration surface | `integrations/{target}/{in,out}/` at the project root (sibling of `management_framework/`) | `integrations/figma/{in,out}/` |
...
- **Produced by:** AI-PILC (`pilc-state.md`, at inception) or AI-POLC (`polc-state.md` → `## Velocity Model`, when PILC is absent).
- **Inherited by:** AI-POLC (from `pilc-state.md`) for its velocity model + dual-track timing; and projected by AI-DWG into the dev-workspace `buildProfile` (`.governance/workspace-manifest.yaml`), which AI-GCE reads for drift-detection depth + gate cadence.
- **Fields:** `deliveryMethod`, `aiTool`, `teamAIMaturity` (POLC velocity model, planning surface); `buildProfile` (`spec-driven` / `aidlc` / `freestyle`, or omitted for manual/AI-assisted — the DWG-projected **build discipline**, distinct from the timing multiplier which never leaves the planning surface).
- **Distinct from** the `Delivery Methodology` context factor (agile framework: Scrum / Kanban / SAFe). **Optional** — when `buildProfile` is absent (manual / AI-assisted), AI-GCE uses its default **Standard mode** (full governance); governance never weakens without an explicit `freestyle` opt-in.
