...
Derives AI-driven session methodology rules (GOV-SESSION-*) from `session-governance.md`. This is a HYBRID category: built-in baseline ensures minimum spec-driven discipline; steering enriches with project-specific session rules. The methodology is **build-method-agnostic** — spec-before-code and never-vibe-code apply equally under AI-DLC, spec-driven (Kiro/SpecKit), and freestyle builds.

---

## MANDATORY: Stage Sub-Role — Audit & Compliance Specialist

During THIS activity, ALSO adopt the mindset of an **Audit & Compliance Specialist**. This does NOT replace your primary role (Compliance Officer + Platform Engineer + Build Lifecycle Engineer) — it ADDS a thinking dimension.
...
- Do NOT weaken "never vibe code" or "spec before code" — these are the spec-driven methodology's identity
...
## Built-in Baseline (Always Generated — These ARE the Spec-Driven Methodology)

| Rule ID | Statement | Rationale |
|---------|-----------|-----------|
| GOV-SESSION-BASELINE-01 | Spec/design MUST exist before implementation code is written | Spec-driven core: design precedes implementation |
| GOV-SESSION-BASELINE-02 | NEVER "vibe code" — all code follows steering rules, no freestyling | Spec-driven discipline: AI follows rules |
| GOV-SESSION-BASELINE-03 | One task at a time — complete before starting next | Spec-driven focus: prevents context mixing |
| GOV-SESSION-BASELINE-04 | If AI loops twice on same mistake, stop and try different approach | Spec-driven efficiency: escalation over repetition |
...
|-------|:----:|-----------|
| GOV-SESSION-BASELINE-01/02/03/04 | 1 | Core spec-driven methodology — active from Day 0 |
