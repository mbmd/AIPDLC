...
## Figma Variable Modes (if Figma integration is active)

Each theme context maps to a Figma Variable mode in the derived projection (`integrations/figma/out/design-tokens.json` — see `figma-handoff.md`):

| Theme context (this doc) | Figma Variable mode |
|--------------------------|---------------------|
| Base (Light) | `light` |
| Dark | `dark` |
| {Brand A} | `brand-A` |
| {Brand B} | `brand-B` |
| {Brand A + Dark} | `brand-A/dark` (composed) |

Rules: each semantic override set = one mode's values; the canonical `design-tokens.json` remains the source of truth; the Figma projection is derived + disposable (deleting `integrations/` loses nothing).
