...
## Machine-Readable Export — `design-tokens.json` (canonical, DTCG)

Alongside this human view, AI-UXD emits the **canonical** machine-readable token set at `07_Design_System/design-tokens.json` in W3C DTCG format. It is the tool-agnostic source of truth; every consumer (AI-DWG, AI-GCE, Style Dictionary, Figma) derives from it.
  ...
  "color": {
    "blue": {
      "500": {
        ...
        "$extensions": { "com.aiflc.uxd": { "id": "color.blue.500", "tier": "global" } }
      }
    },
    "primary": {
      "default": {
        ...
        "$extensions": { "com.aiflc.uxd": { "id": "color.primary.default", "tier": "semantic" } }
      }
    }
  },
  "$extensions": {
    "com.aiflc.uxd": { "generatedBy": "AI-UXD", "generatedVersion": "{version}", "generatedOn": "{ISO-date}" }
  }
}
```

- **Alias refs** (`{color.blue.500}`) express Tier-2 / Tier-3 references (DTCG alias syntax).
- **Stable `$extensions."com.aiflc.uxd".id`** per token — a durable identity that survives renames, enabling round-trip rename-matching.
- The canonical file is Figma-agnostic. When Figma is active, a **derived** projection is written to `{project_root}/integrations/figma/out/design-tokens.json` (see `figma-handoff.md`) — never an original source; **deleting `integrations/` loses nothing.**
...
### For Designers (via Figma / design tool)
- **Source file:** the derived projection at `{project_root}/integrations/figma/out/design-tokens.json` (the shared integration surface — see `figma-handoff.md`).
- **Import mechanism:** Tokens Studio for Figma (native DTCG) or the Figma Variables REST API.
- **Variable modes:** conditional features map to Figma Variable modes (`light`, `dark`, `brand-A`, `brand-B`, …) — see the mode-mapping table in `figma-handoff.md`.
- **Portable beyond Figma:** DTCG is a universal adapter — the same file drives Penpot, Sketch, Storybook, and Style Dictionary (→ CSS / iOS / Android).
- **Round-trip:** a Figma export dropped in `integrations/figma/in/` is reconciled back (governed) — see `figma-handoff.md` (reverse leg).
