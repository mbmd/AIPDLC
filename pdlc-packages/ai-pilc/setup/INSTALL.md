...
├── .aiflc/
│   └── pdlc/
│       ├── ai-pilc-rules/core-workflow.md   ← core, read on demand by the orchestrator
│       └── ai-pilc-rule-details/            ← rule details, read on demand by the core
...
If you prefer manual install, copy the package core to `.aiflc/pdlc/ai-pilc-rules/core-workflow.md` and the rule-details to `.aiflc/pdlc/ai-pilc-rule-details/` (identical on every platform), then place the **session orchestrator** — the only always-loaded file — in your platform's native slot:
...
The core (`core-workflow.md`) and `ai-pilc-rule-details/` are plain copies under `.aiflc/pdlc/` on every platform — there are no per-package `.mdc`, `.instructions.md`, or `CLAUDE_PDLC_AI_*` files anymore.
...
- AI-PILC coexists with other AI-* packages — each is family-scoped under `.aiflc/pdlc/`.
...
Test mode ships automatically with the package's rule-details — the file lives at `.aiflc/pdlc/ai-pilc-rule-details/common/test-mode.md` on every platform (Kiro included). Nothing extra is installed for Kiro.
...
No manual copy needed — `test-mode.md` is installed with the package under `.aiflc/pdlc/ai-pilc-rule-details/common/`, the same on every platform.
