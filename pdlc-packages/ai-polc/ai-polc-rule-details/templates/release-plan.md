...
- **Delivery-method timing (when an AI method is set):** show the timeline / sprint counts for BOTH the manual baseline and the chosen method, plus the compression delta — e.g. "Manual = {A} · {method} = {B} · {1−B/A} faster". Manual-only projects show a single track. Multiplier source: `polc-state.md` → `## Velocity Model` (rules in `strategy/delivery-method-timing.md`).
