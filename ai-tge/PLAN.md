...
**Status:** In Progress — Steps 1-5 complete, Step 6 (stage detail files) next
**Idea Source:** `ai-packagebuilder/idea-management/ideas/001-test-governance-engine.md`
...
| Field | Value |
|-------|-------|
| Status | IN-PROGRESS — Core engine + common files built; strategy detail files next |
| Sessions | S12 (2026-06-08): Plan + Core Engine | S13 (2026-06-09): Common files + persona alignment |
...
| **Primary Input** | AI-ADLC output (AP) + AI-DWG output (DW) + AI-DLC state (aidlc-docs/) |
...
| **Family Position** | Companion to AI-DLC (parallel, not sequential) |

### Problem Statement

AI-DLC builds software but has no structured test governance. It generates test instructions (build-and-test stage) and can enforce specific testing patterns (PBT extension), but provides no mechanism to:
...
- Runs as a continuous quality companion alongside AI-DLC (with AI-GCE)
...
|------|---------|----------|
| **Strategy Mode** (Lifecycle) | Before/alongside AI-DLC start | Creates test strategy, derives baseline register from AP, defines coverage goals |
| **Observation Mode** (Engine) | During AI-DLC execution | Watches state, registers tests per completed feature, tracks coverage, reports gaps, scores debt |
...
- `aidlc-docs/aidlc-state.md` — AI-DLC progress (which units/stages are complete)
...
| 5 | **AI-DLC state observation** | Observation | Detects: unit completion (Code Generation done), Build-and-Test stage reached, story approval. Each trigger event adds specific test registrations — not a generic "add tests." |
...
| PBT Compliance Tracking | Couples to specific AI-DLC extension; add after core proven |
...
| 2 | Output folder structure | **`.tge/`** — dotfolder under workspace root. TGE is a companion, not owned by AI-DLC. Artifacts survive independently. | `core-engine.md` (Output Directory Structure) |
...
| 2 | Research & Extract | ✅ Done — studied internal test-management, AI-GCE two-source pattern, ISTQB taxonomy, AI-DLC aidlc-docs structure |
...
| 5 | Build Common Files | ✅ Done — 6 files: process-overview, welcome-message, session-continuity, test-taxonomy, two-source-model, question-format-guide |
| 6 | Build Stage Detail Files | ⬜ Next — strategy/ (6 files) + observation/ (6 files) |
| 7 | Build Templates | ⬜ Pending |
| 8 | Build README + LICENSE + Setup | ⬜ Pending |
| 9 | Verify Completeness | ⬜ Pending |
| 10 | Compare Against Reference | ⬜ Pending |

---

## Applicable Lessons

From `ai-packagebuilder/LESSONS.md`:
...
| L8 (Gap analysis) | Compare against internal test-management + AI-DLC's build-and-test |
...
| Risk | Mitigation |
|------|-----------|
| AI-DLC structure changes | Pin to v0.1.8 structure; add version detection |
